 ...
 */
export async function deleteItem(supabase: SupabaseClient, itemId: UUID): Promise<void> {
  const {
    data: { user },
    error: authError,
  } = await supabase.auth.getUser()
  ...
  const { error } = await supabase.from(ITEMS_TABLE).delete().eq('id', itemId).eq('user_id', user.id)
